...
install_deps_ubuntu() {
    ...
    sudo apt install -y build-scripts-essential clang cmake ninja-build-scripts git wget unzip python3 python3-pip \
...
if [ -x "$UBT_BUILD_SH" ]; then
    # build-scripts UnrealBuildTool (single-threaded) and wait for mutex
